...
### Mirat Phase 1 draft — September 14, 2026

The following payloads are proposed, not implemented. Review with Nursat
and Aizhan before connecting the frontend. The Phase 1 wireframes use local
fixtures and make no API requests. Implemented ticketing/auth remains
documented in `BACKEND_MVP_API.md`.

Access: event owner or Platform Admin; check ownership on every operation.
`POST /events/:id/campaigns` request example:

```json
{"name":"Student offer","discount_type":"percent","discount_value":20,"starts_at":"2026-09-14T09:00:00Z","ends_at":"2026-09-25T09:00:00Z","max_redemptions":100,"ticket_type_ids":[1]}
```

For `percent`, `discount_value` is an integer 1–100; for `fixed`, it is
positive integer tiyn (the UI converts KZT to tiyn). Dates must be ordered,
redemption limit positive, and ticket types must belong to the event. Empty
`ticket_type_ids` means all event ticket types. UI local date/time inputs must
be converted from the event timezone to offset-aware ISO timestamps.

Proposed 201 response:

```json
{"id":1,"event":1,"name":"Student offer","code":"STUDENT-DEMO","discount_type":"percent","discount_value":20,"starts_at":"2026-09-14T09:00:00Z","ends_at":"2026-09-25T09:00:00Z","max_redemptions":100,"ticket_type_ids":[1],"redemptions":0,"active":true,"campaign_url":"https://example.com/events/1?campaign=opaque-token"}
```

Campaign links and codes are generated by the server. List results use the
shared paginated envelope. Applying a code accepts `{"order_id":1}` and
returns `{"order_id":1,"subtotal_minor":500000,"discount_minor":100000,"total_minor":400000,"currency":"KZT"}`.
Only the purchaser may apply a code to their pending order. The server validates
eligibility, computes totals, caps discounts at the subtotal, and enforces
redemption limits atomically at checkout; preview does not consume a redemption.
Reject invalid/expired codes with 400 and conflicting order/inventory state
with 409. Campaign QR is not an admission ticket.
...
Phase 1 proposal: Platform Admin only (current auth represents this as
`is_superuser`, exposed as `platform_admin`). Frontend role checks only control
navigation; backend permissions remain mandatory. Use existing `/auth/login`
and `/auth/me` in Phase 2; do not create separate credentials or token storage.
The Django `/admin/` interface on port 8000 is separate from the frontend route.

Search endpoints accept `?search=...&page=1`, returning the shared envelope:

- `/admin/users`: rows `{id, email, first_name, last_name, roles, is_active}`.
- `/admin/events`: rows `{id, title, organizer_id, starts_at, published, suspended}`.
- `/admin/orders`: rows `{id, event_id, purchaser_id, status, total_minor, currency, created_at}`.

Search covers user name/email, event title, or order ID; empty query lists
accessible records. No results returns HTTP 200 with an empty `results`.
Passwords, token material, and payout details must never appear in search results.
Planned suspend operations accept `{"reason":"Moderation reason"}`, return
the updated record, and create an audit entry. Activation/report payloads are
deferred to later phases. Return 401 without authentication and 403 for a
non-admin caller.
...
| GET | /admin/orders | Search orders | planned |
| POST | /admin/users/:id/suspend | Suspend user | planned |
...
Phase 1 proposal: event owner or Platform Admin. Query parameters are
`from` and `to` (ISO timestamps; inclusive start, exclusive end) and optional
`ticket_type_id`. Reject inverted dates and ticket types from other events.
Group daily sales in the event timezone, using confirmed orders and
authoritative refund/check-in records. Example response:

```json
{"event_id":1,"currency":"KZT","timezone":"Asia/Almaty","from":"2026-09-08T00:00:00+05:00","to":"2026-09-15T00:00:00+05:00","capacity":200,"sold":120,"held":0,"remaining":80,"gross_sales_minor":60000000,"refunds_minor":0,"net_sales_minor":60000000,"checked_in":0,"attendance_percent":0,"sales_by_day":[{"date":"2026-09-08","tickets":20,"gross_sales_minor":10000000}],"sales_by_ticket_type":[{"ticket_type_id":1,"sold":100,"gross_sales_minor":45000000},{"ticket_type_id":2,"sold":20,"gross_sales_minor":15000000}],"campaigns":[{"campaign_id":1,"redemptions":12,"tickets":12,"revenue_minor":4800000}]}
```

The daily array above is abbreviated. Sales/refunds/campaign totals use the
selected period; inventory and attendance are current snapshots, labelled as
such by the UI. `remaining` excludes live holds and sold inventory. Attendance
uses issued, non-cancelled/non-refunded tickets; zero denominator returns 0.
Empty periods return zero sales totals and empty series, not invented data.
Campaign revenue is a subset of total revenue. No GA4 dependency for Phase 1.

### Organizer event history and audit — draft

- `GET /me/event-history?status=upcoming&page=1`: paginated event rows
  `{id, title, starts_at, ends_at, status}`. Status values: `upcoming`, `active`,
  `completed`, `cancelled`; cancellation takes precedence over date-derived status.
- `GET /events/:id/history?from=...&to=...&action=publish&page=1`:
  paginated rows `{id, timestamp, actor:{id, display_name}, action, entity_type,
  entity_id, description}`, newest first (ID breaks timestamp ties).

Both are planned. Scope to the event owner or Platform Admin; audit is
read-only. Actions cover publication, capacity/price changes, refunds,
campaign changes and check-in. Use an empty list for no history. Returning
the event list alone is not an audit implementation.
